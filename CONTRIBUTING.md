...
PairCue supports Python 3.11, 3.12, and 3.13. From the repository root:
...
Run PairCue locally with `paircue setup` or inspect the CLI using `paircue --help`.
...
ruff check .
mypy src/paircue
pytest --cov=paircue --cov-report=term-missing --cov-fail-under=60
...
- Do not copy, port, translate, decompile, or closely adapt code from another subtitle product.
  Implement from PairCue's specifications, public standards, and official API documentation.
